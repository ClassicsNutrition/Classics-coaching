...
export default function AdminEbookEditorPage({ params }: Props) {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from('ebooks').select('*').eq('slug', slug).single();
      if (data) {
        setEbook(data);
      ...
      <aside style={{ width: 300, borderRight: '1px solid rgba(255,45,120,0.1)', background: 'rgba(6,6,15,0.95)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: 24, borderBottom: '1px solid rgba(255,45,120,0.1)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            <div style={{ width: 38, height: 38, borderRadius: 10, background: 'linear-gradient(135deg, #FF2D78, #7B2FBE)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
              <BookOpen size={20} />
            </div>
            <div>
              <h1 style={{ color: 'white', fontSize: '1rem', fontWeight: 800, margin: 0, fontFamily: 'var(--font-display)' }}>{ebook.title}</h1>
              <p style={{ color: 'rgba(226,232,240,0.4)', fontSize: '0.75rem', margin: 0 }}>Édition E-book</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <button onClick={() => setTab('editor')} style={{ 
            ...
            background: tab === 'editor' ? 'rgba(255,45,120,0.15)' : 'transparent',
          ...
          <button onClick={() => setTab('access')} style={{ 
            display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 12, border: 'none', cursor: 'pointer',
            background: tab === 'access' ? 'rgba(255,45,120,0.15)' : 'transparent',
        ...
        <div style={{ marginTop: 'auto', padding: 20, borderTop: '1px solid rgba(255,45,120,0.1)' }}>
          <button onClick={handleSave} disabled={saving} className="btn-primary" style={{ width: '100%', justifyContent: 'center' }}>
        ...
        {tab === 'editor' ? (
          <div style={{ maxWidth: 900, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontFamily: 'var(--font-display)', fontSize: '1.5rem', fontWeight: 800 }}>Construction du contenu</h2>
              <button 
                onClick={() => setPreview(!preview)} 
                className="btn-ghost" 
                style={{ fontSize: '0.85rem', color: preview ? 'var(--miami-pink)' : 'white' }}
              >
            ...
            {preview ? (
              <div className="card-glass" style={{ padding: 40 }}>
                <EbookRenderer blocks={blocks} themePrimary={ebook.theme_primary} themeAccent={ebook.theme_accent} />
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {blocks.map((block, i) => (
                  <div key={block.id} className="card-glass" style={{ padding: 0, overflow: 'hidden', border: editingId === block.id ? '1px solid var(--miami-pink)' : '1px solid rgba(255,45,120,0.1)' }}>
                    <div 
                    ...
                    {editingId === block.id && (
                      <div style={{ padding: 24, borderTop: '1px solid rgba(255,45,120,0.08)' }}>
                ...
                <div style={{ position: 'relative', marginTop: 20 }}>
                  <button onClick={() => setShowAddMenu(!showAddMenu)} className="btn-ghost" style={{ width: '100%', borderStyle: 'dashed', justifyContent: 'center', border: '2px dashed rgba(255,45,120,0.2)', padding: 16 }}>
                    <Plus size={18} /> Ajouter un bloc de contenu
                  </button>
                  {showAddMenu && (
                    <div style={{ position: 'absolute', top: '110%', left: 0, right: 0, zIndex: 100, background: 'var(--miami-night)', border: '1px solid rgba(255,45,120,0.2)', borderRadius: 16, padding: 12, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
                      {(['heading', 'text', 'exercise', 'table', 'callout', 'divider'] as BlockType[]).map(type => (
                        ...
                        }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,45,120,0.1)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.03)'}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[1, 2, 3].map(l => (
              <button key={l} onClick={() => onUpdate({ level: l })} style={{ 
                ...
                background: block.level === l ? 'rgba(255,45,120,0.1)' : 'transparent',
                color: block.level === l ? 'var(--miami-pink)' : 'rgba(226,232,240,0.4)',
